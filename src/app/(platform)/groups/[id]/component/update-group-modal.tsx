...
const UpdateGroupModal = ({ isOpen, onClose, onSuccess, groupId, initialData }: UpdateGroupModalProps) => {
    const [isSubmitting, setIsSubmitting] = useState(false);

    const [formData, setFormData] = useState({
        name: initialData.name,
        visibility: initialData.visibility,
        code_enabled: initialData.code_enabled,
        contest_coordinator_type: initialData.contest_coordinator_type,
        description: initialData.description,
    });

    // Sync form data when initialData changes (e.g. modal re-opens with fresh data)
    useEffect(() => {
        if (isOpen) {
            setFormData({
                name: initialData.name,
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await groupService.updateGroup({
                group_id: groupId,
                name: formData.name,
                visibility: formData.visibility.trim().toLowerCase() as any,
                code_enabled: formData.code_enabled,
                contest_coordinator_type: formData.contest_coordinator_type as any,
            ...
            const errorMessage = error?.response?.data?.message || "Failed to update group. Please try again.";
                    ...
                    <div className="flex items-start justify-between p-6 pb-4">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1">
                                Modify your group settings and details.
                            </p>
                        </div>
                        <button
                            onClick={onClose}
