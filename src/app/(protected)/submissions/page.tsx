...
export default function SubmissionsPage() {
  ...
  const handleViewChange = (type: 'all' | 'mine') => {
    ...
    setFilters(prev => ({ 
      ...prev, 
      userHandle: type === 'mine' ? (session?.user?.handle || session?.user?.name || "") : '' 
    }));
  };
    ...
    <div className="container mx-auto px-4 py-18">
      <div className="flex gap-8 p-8">
        <div className="w-48 flex flex-col gap-2 ">
          <button onClick={() => handleViewChange('all')} className={`p-3 rounded text-left ${viewType === 'all' ? 'bg-blue-900 text-white' : 'hover:bg-gray-100'}`}>All</button>
          <button onClick={() => handleViewChange('mine')} className={`p-3 rounded text-left ${viewType === 'mine' ? 'bg-blue-900 text-white' : 'hover:bg-gray-100'}`}>Mine</button>
        </div>
          ...
          filters={
            <div className="flex flex-wrap gap-6 p-5 bg-white border border-gray-200 rounded-xl shadow-sm items-end">
              <div className="flex flex-col">
                ...
                <input className="border border-gray-300 rounded-md p-2 w-48 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Search user..." value={filters.userHandle} onChange={(e) => setFilters(prev => ({...prev, userHandle: e.target.value}))} />
              </div>

              <div className="flex flex-col">
                <label className="text-xs font-bold text-gray-500 uppercase mb-2">OJ</label>
                <select className="border border-gray-300 rounded-md p-2 w-32 bg-white focus:ring-2 focus:ring-blue-500 outline-none" value={filters.online_judge || ''} onChange={(e) => setFilters(prev => ({...prev, online_judge: e.target.value}))}>
              ...
              <div className="flex flex-col">
                ...
                <select value={filters.language} onChange={(e) => setFilters(prev => ({...prev, language: e.target.value}))} className="border border-gray-300 rounded-md p-2 w-32 bg-white focus:ring-2 focus:ring-blue-500 outline-none">
                  <option value="">All</option>
