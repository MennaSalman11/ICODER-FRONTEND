...
console.log("TOKEN:", token);
  ...
  const [createdSubmissionId, setCreatedSubmissionId] = useState<number | null>(
    null
  );
...
const streamStarted = useRef(false);  // Sync Monaco editor code + language
  ...
  const fetchSessionStatus = async () => {
    ...
    try {
      ...
      if (data) {
        const activeName =
          session?.user?.name || data.handle || "User Account";
  ...
  const getBannerStyles = (verdict: string) => {
    ...
    if (
      ...
      return {
        bg: "bg-red-50 border-red-200",
        text: "text-red-700",
        label: v === "FAILED" ? "Failed" : "Runtime Error",
        icon: <XCircle size={14} className="text-red-600" />,
      };
    }
    ...
    if (v === "COMPILATION_ERROR" || v === "CE") {
      return {
        ...
        icon: <Terminal size={14} className="text-slate-600" />,
      };
    }
    ...
    if (v === "MEMORY_LIMIT_EXCEEDED" || v === "MLE") {
      return {
        ...
        icon: <XCircle size={14} className="text-cyan-600" />,
      };
    }
...
useEffect(() => {
  ...
  const onSubmitCode = async (
    ...
    if (
      ...
      toast.error(
        "Please configure your account Session ID first."
      );
      return;
    }
    ...
    try {
      ...
      const payload = {
        user_id: userId,
        problem_code:
          (params?.code || data.problem_code) as string,
      ...
      if (response?.id) {
        toast.success(
          "Solution pushed successfully! Waiting for verdict...",
          { id: toastId }
        );
    ...
    <div className="w-full text-left bg-white">
      ...
      {createdSubmissionId && submissionResult ? (
        <div className="space-y-4 animate-in fade-in duration-300">
          ...
          <div className="border border-gray-100 rounded-xl overflow-hidden text-[11px] shadow-xs">
            ...
            <div className="grid grid-cols-5 p-2.5 text-center items-center font-semibold text-gray-700">
              <div
                className={`py-1 px-2 rounded-lg mx-1 font-bold flex items-center justify-center gap-1 border ${
                  getBannerStyles(submissionResult.status).bg
                } ${
                  getBannerStyles(submissionResult.status)
                    .text
                }`}
              >
                {
        ...
        <div className="space-y-5 animate-in slide-in-from-bottom-2 duration-200">
          ...
          <div className="bg-blue-50/60 border border-blue-100 p-3.5 rounded-xl text-xs text-blue-800 flex items-start gap-2 leading-relaxed">
            ...
            <span>
              ...
              with your own account, and fill
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              >
                ...
                >
                  <div
                    className={`bg-white w-3 h-3 rounded-full transition-transform ${
                      isOpened
                        ? "translate-x-3"
                        : "translate-x-0"
                    }`}
          ...
          {/* Code */}
          <div className="flex flex-col gap-1.5">
            <Label className="text-gray-700 font-bold text-[11px] uppercase tracking-wider">
              Source Code
            </Label>
            ...
            <div className="relative">
              ...
              {errors.code && (
                <p className="text-red-500 text-[10px] mt-1 italic font-semibold ml-1">
