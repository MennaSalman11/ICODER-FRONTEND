...
export const getLanguageList = async () => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/coding/editor/languages`) 
    if (!res.ok) {
        return {
            status: res.status,
...
export const getLanguageById = async (id: string) => {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/coding/editor/language?${query.toString()}`,{
        headers: {
...
export const submitCode = async (SubmissionPayload: SubmissionPayload) => {
    ...
    const res = await fetch (`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/coding/editor/submissions`,{
        method: 'POST',
        headers: {
            'content-type': 'application/json',
...
export const submitBatchCode = async (batchPayload: BatchSubmissionPayload) => {
    const { token } = await getUserToken();
    const res = await fetch (`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/coding/editor/submissions/batch`,{
        method: 'POST',
        headers: {
            'content-type': 'application/json',
            'Authorization': `Bearer ${token}`
...
const params = new URLSearchParams();
    tokens.forEach(t => params.append('tokens', t));
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/coding/editor/submissions/batch?${params.toString()}`, {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${token}`
        }
